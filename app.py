...
# --- NAMA KOLOM KONSTAN (# FILE: app.py (FINAL KOREKSI UNTUK KEYERROR PADA SCALING)
import streamlit as st
import pandas as pd
import numpy as np
import joblib
...
try:
    ...
    # Ambil daftar nama kolom dari scaler untuk memastikan urutan dan string yang benar
    FEATURE_NAMES = list(scaler.feature_names_in_)
    
    # Ambil nama kolom yang bermasalah dari daftar yang dipelajari Scaler
    # Cari nama kolom yang mengandung 'suicidal thoughts'
    COL_SUICIDAL = [col for col in FEATURE_NAMES if 'suicidal thoughts' in col][0]
    COL_SLEEP = [col for col in FEATURE_NAMES if 'Sleep Duration' in col][0]
    COL_FINANCIAL = [col for col in FEATURE_NAMES if 'Financial Stress' in col][0]
    COL_FAMILY = [col for col in FEATURE_NAMES if 'Family History' in col][0]
    COL_STUDY_SAT = [col for col in FEATURE_NAMES if 'Study Satisfaction' in col][0]
    
    # Tipe data ini hanya untuk tampilan SelectBox
...
if st.button("Prediksi Potensi Depresi"):
    # 1. Kumpulkan data input. Kunci dictionary harus mengikuti nama yang dipelajari Scaler.
    data = {
        'Gender': [gender_input], 
        'Age': [age], 
        'City': [city_input], 
    ...
    # d. Scaling (Menggunakan urutan kolom yang benar dari Scaler)
    ...
    with col_res2:
        ...
        st.metric("Probabilitas Tidak Depresi (Kelas 0)", f"{prediction_proba[0]:.2%}")HARUS SAMA PERSIS DENGAN DATASET) ---
